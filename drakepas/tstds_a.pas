Program TestDrakeSnake_Anfang;

Uses Crt,FlyAfrik,Kart3Flg;
      ...
      Story : Array[0..Seiten-1,0..8] of String =
               (('Wir schreiben den 1. April',
                ...
                ('Volk das nicht wei· warum es',
                 ...
                 'NÑhe von Mechico / Amazona·',
                 'befinden.',
                 'Dort herrschte einmal ein',
                 'Kînig mit dem Namen Hechtel-',
                 'Zechtel-Obo-Boppel-Kattel'),
                ('Er soll sagenhafte ReichtÅmer',
                 'in den GÑngen seines Reiches',
                 'versteckt haben sowie viele',
                 'Fallen und Feinde.',
                 '',
                 '',
                 '',
                ...
                ('Nachdem Drake Snake sich das',
                 ...
                 'nach Mechico / Amazona· um',
                 'diesen sagenhaften Schatz zu',
                 'finden, um sich endlich ein',
                 'schînes Leben machen zu',
                 'kînnen ...'),
                ('',
                 '',
                 '',
...
Procedure LoadLowresPic(Name : String;Var Farben : FarbLowrestyp;
  ...
  Begin
    ...
    For p:=0 to 3 do Begin
      WritePlane(p);
      BlockRead(f,Mem[Screen:Ofset],Menge);
    End;
...
Begin
  ...
  LoadLowresPic('COD.BLD',LPalette,19200,19200);  { COD-Logo laden }
  ...
  For y:=0 to 95 do
    LPal2[y]:=0;
  SetLowresPalette(LPal2);
  LoadLowresPic('PRESENT1.BLD',LPalette,0,16000);  { PRESENT laden }
  ...
  LoadHiresPic('DR_SNAKE.BLD',Palette);     { Drake Snake Titelbild laden }
  ...
  For p:=0 to 47 do Palette[p]:=0;
  SetHiresPalette(Palette);
  LoadHiresPic('BUCH.BLD',Palette);         { Bild des Alten Buches laden }
  For x:=0 to 128 do Begin                  { und einblenden }
    For y:=0 to 47 do
      Pal2[y]:=(Palette[y]*x) shr 7;
    SetHiresPalette(Pal2);
  End;
  WaitKey(9000);                            { ca. 9 sekunden warten }
  For x:=128 downto 0 do Begin              { und wieder ausblenden }
    For y:=0 to 47 do
      Pal2[y]:=(Palette[y]*x) shr 7;
    SetHiresPalette(Pal2);
