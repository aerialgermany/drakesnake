...
Procedure SetPalette(Pal : Paltyp;AnzFarben : Word);
                                                  {A Setzen der Farbpalette }
Procedure SetPaletteDS(Pal : Paltyp);
...
Procedure LoadImage(Name : String; Adresse : Pointer);        { Image laden }
...
Function  ReadPlane(Plane : Byte;Offset : Word) : Byte;
                                      { Verwaltet Modus 13h als 8 Bitplanes }
Procedure SaveIFF(BildName : String;Bildspeicher : Pointer;
                      Strukt : IFFStruct);
...
{    mov   DX,3C4h             Bildschirm abschalten
    ...
    push  DS
    mov   CX,AnzFarben            { 256 Farbeintr„ge }
    mov   DS,Segment
    mov   BX,Off
    mov   DX,3C8h
    mov   AL,0
...
Procedure LoadImage(Name : String; Adresse : Pointer);
...
Begin
  ...
  Assign(f,Name);
  Reset(f,1);
  BlockRead(f,Breite,2,result);
  BlockRead(f,Hoehe,2,result);
  Mem[AdrHi:AdrLo]:=Breite;
  Mem[AdrHi:AdrLo+2]:=Hoehe;
  Menge:=Breite*Hoehe;
  BlockRead(f,Mem[AdrHi:AdrLo+4],Menge,result);
  Close(f);
End;
...
Const FORM : Array[0..3] of Char = ('F','O','R','M');
      ILBM : Array[0..3] of Char = ('I','L','B','M');
      BMHD : Array[0..3] of Char = ('B','M','H','D');
      CMAP : Array[0..3] of Char = ('C','M','A','P');
      BODY : Array[0..3] of Char = ('B','O','D','Y');
...
Begin
  ...
  Assign(Pic,BildName);
  ReWrite(Pic,1);
  BlockWrite(Pic,FORM,4);
  Lform:=56+((1 shl (Strukt.BitPlanes-1))*3)+
             Strukt.BitPlanes*Strukt.Breite*Strukt.Hoehe;
  BlockWrite(Pic,Lform,4);
  BlockWrite(Pic,ILBM,4);
  BlockWrite(Pic,BMHD,4);
  { L„nge der BMHD / Long }
  Lbmhd:=20; BlockWrite(Pic,Lbmhd,2);
  { Breite der Grafik / Word }
  BreiteG:=Strukt.Breite; BlockWrite(Pic,BreiteG,2);
  { H”he der Grafik / Word }
  HoeheG:=Strukt.Hoehe; BlockWrite(Pic,HoeheG,2);
  { X-Position der Grafik / Word }
  XG:=0; BlockWrite(Pic,XG,2);
  { Y-Position der Grafik / Word }
  YG:=0; BlockWrite(Pic,XG,2);
  { Anzahl der Bitplanes }
  BlockWrite(Pic,Strukt.BitPlanes,1);
  { Masking (0 - Kein Masking / 1 - Masking / 2 - Transparent / 3 - Lasso) }
  Mask:=0; BlockWrite(Pic,Mask,1);
  { DatenKompression (0 - nicht gepackt / 1 - gepackt) }
  Kompress:=0; BlockWrite(Pic,Kompress,1);
  { unbenutzt }
  None:=0; BlockWrite(Pic,None,1);
  { Transparentfarbe / Word }
  TransC:=0; BlockWrite(Pic,TransC,2);
  { X-Aspekt }
  XAsp:=1; BlockWrite(Pic,XAsp,1);
  { Y-Aspekt }
  YAsp:=1; BlockWrite(Pic,YAsp,1);
  { Breite der Quellseite }
  BreiteQ:=Strukt.Breite; BlockWrite(Pic,BreiteQ,2);
  { H”he der Quellseite }
  HoeheQ:=Strukt.Hoehe; BlockWrite(Pic,HoeheQ,2);
  BlockWrite(Pic,CMAP,4);
  Lcmap:=(1 shl (Strukt.BitPlanes-1))*3;
  { L„nge der CMAP / Long }
  BlockWrite(Pic,Lcmap,4);;
  BlockWrite(Pic,Strukt.Farben,Lcmap);
  BlockWrite(Pic,BODY,4);
  Lbody:=Strukt.BitPlanes*Strukt.Breite*Strukt.Hoehe;
  BlockWrite(Pic,Lbody,4);
  BlockWrite(Pic,Bild2,Lbody);
End;
